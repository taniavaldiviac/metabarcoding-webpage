...
```
{locus}_{sample_id}_S{N}_L{lane}_R{1|2}_001.fastq
```
...
|:------|:--------|:------------|
| `{locus}` | `12S` | Debe coincidir exactamente con `locus_shorthand` en `primer_data.csv` |
| `{sample_id}` | `GC-001-d1` | Identificador único de la muestra — debe estar en `metadata.csv` |
| `S{N}` | `S1` | Número de muestra asignado por el secuenciador |
| `L{lane}` | `L001` | Carril de secuenciación |
