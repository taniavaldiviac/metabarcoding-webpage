...
Carga el checkpoint guardado al final del filtrado de calidad. Como es una sesión nueva, primero define las rutas y luego busca el archivo automáticamente:
...
checkpoint_file <- tail(sort(
  list.files(file.path(output_location, "rdata_output"),
             pattern = "_checkpoint2_Dereps\\.RData$",
             full.names = TRUE)
), 1)

stopifnot("No se encontró checkpoint de dereplicación" = length(checkpoint_file) > 0)
load(checkpoint_file)
message("Checkpoint cargado: ", basename(checkpoint_file))
```
...
**Qué hace:** estima las tasas de error específicas de **tu** run de secuenciación. La secuenciadora Illumina comete errores que dependen del tipo de transición (A→C, A→G, etc.) y del Q-score de cada posición. `learnErrors()` aprende estas tasas directamente de tus datos — iterando hasta que las estimaciones estabilicen — para no usar valores genéricos que no reflejan el perfil real de tu equipo y reactivos.
