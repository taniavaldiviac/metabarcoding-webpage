...
message("Truncamiento ajustado: F=", where_trim_all_Fs, " | R=", where_trim_all_Rs)
```

### Paso 3: Verificación de solapamiento

Finalmente verificamos que las lecturas truncadas se solapen lo suficiente para que DADA2 pueda unirlas (merge). El solapamiento se calcula como:
...
```{r eval=FALSE}
overlap <- where_trim_all_Fs + where_trim_all_Rs - amp_length
message("Solapamiento esperado: ", overlap, " bp")

if (overlap < 12) {
  stop("Solapamiento < 12 bp. El merge fallará. Aumenta truncLen o revisa el marcador.")
} else if (overlap < 20) {
  warning("Solapamiento < 20 bp. El merge puede ser inestable.")
} else {
  message("Solapamiento suficiente para el merge.")
}
```
...
| Marcador | Amplicón | truncLen F/R | Solapamiento | Situación |
|:---------|:---------|:-------------|:-------------|:----------|
| **12S (nuestros datos)** | ~65 bp | 65 / 65 | 65 bp | Solapamiento total — ideal |
| **16S** | ~150 bp | 140 / 130 | 120 bp | Amplio solapamiento |
| **COI** | ~313 bp | 250 / 200 | 137 bp | Buen solapamiento |
| **COI** (calidad baja) | ~313 bp | 200 / 150 | 37 bp | Ajustado pero funcional |
| **COI** (muy recortado) | ~313 bp | 150 / 120 | -43 bp | No hay solapamiento — el merge falla |

::: {.callout-note icon="false"}
**Con amplicones cortos** (como nuestro 12S), el truncamiento está determinado por la longitud del amplicón, no por la calidad — las lecturas son tan cortas que la calidad se mantiene alta durante todo el amplicón.

**Con amplicones largos** (como COI), el truncamiento está determinado por la calidad — hay que buscar el balance entre quitar bases de baja calidad y mantener suficiente solapamiento para el merge.
:::
