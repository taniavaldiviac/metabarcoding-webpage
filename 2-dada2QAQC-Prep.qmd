...
Antes de comenzar el análisis, cargamos las librerías que usaremos a lo largo del pipeline. Cada una cumple un rol específico:

- **dada2**: el motor del pipeline — infiere ASVs, filtra lecturas, modela errores y asigna taxonomía
- **tidyverse**: manipulación y visualización de datos (incluye `dplyr`, `ggplot2`, `readr`, etc.)
- **ShortRead**: lectura y control de calidad de archivos FASTQ
- **here**: manejo de rutas relativas al proyecto, independiente de dónde ejecutes el script
- **seqinr** y **digest**: utilidades para trabajar con secuencias FASTA y generar identificadores únicos (hashes) para las ASVs
...
Definimos las rutas principales del proyecto. Usamos `here::here()` para que el script funcione sin importar desde qué directorio se ejecute — detecta automáticamente la raíz del proyecto (donde está el `.Rproj`).
...
En lugar de elegir manualmente dónde truncar, el script lo calcula automáticamente. Usa una **ventana móvil** de 10 posiciones sobre la calidad mediana: cuando la calidad promedio de la ventana cae por debajo de Q30, marca ese punto como truncamiento.

El valor se limita por `max_trim` de `primer_data.csv` para garantizar que quede suficiente solapamiento entre las lecturas F y R para el merge posterior.
