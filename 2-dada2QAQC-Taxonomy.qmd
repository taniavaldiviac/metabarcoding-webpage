...
3. **Por qué "ingenuo"** — Se llama "naive" porque asume que cada 8-mer es *independiente* de los demás, lo cual no es biológicamente cierto, pero hace el cálculo computacionalmente muy eficiente y funciona bien en la práctica.
...
- `tryRC = TRUE`: prueba también el complemento reverso de cada ASV — importante para marcadores que pueden amplificarse en cualquier orientación
- `outputBootstraps = TRUE`: devuelve los valores de confianza por nivel taxonómico
- La base de datos está definida en `primer_data.csv` (columna `db_name`)
...
El pipeline guarda todos los objetos necesarios para construir el objeto phyloseq en un único archivo `.RData`. Esto permite continuar el análisis en otra sesión sin repetir los pasos anteriores.
...
Exportamos las secuencias de ASVs en formato FASTA para usarlas en análisis posteriores como el agrupamiento en OTUs con VSEARCH.
...
Este archivo contiene las secuencias sin taxonomía — solo un identificador (`ASV_0001`, `ASV_0002`, ...) y la secuencia. VSEARCH lo usará para agrupar ASVs similares en OTUs.
...
| `minBoot` muy alto | Muchos NA en niveles medios | Bajar a 60 con precaución |
...
:::{.callout-note}
## Recuerda
La asignación taxonómica depende de la calidad y cobertura de la base de datos. Ningún método puede asignar correctamente un organismo que no está representado en la referencia. Siempre reporta la versión de la base de datos que usaste en la sección de métodos.
:::

:::{.callout-tip}
## Para el curso
1. ¿Qué porcentaje de ASVs se asignan a nivel de filo? ¿Y a nivel de género?
2. ¿Hay ASVs sin asignación a ningún nivel? ¿Cuántos son?
3. Revisa los valores de bootstrap en `ready_new_taxa` — ¿Hay asignaciones con baja confianza?
