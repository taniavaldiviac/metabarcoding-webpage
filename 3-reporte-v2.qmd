...
# Separar para ejes independientes
df_asv <- df_comp |> dplyr::filter(Estrategia == "ASVs")
df_otu <- df_comp |> dplyr::filter(Estrategia == "OTUs")

# Factor de escala para eje secundario
scale_factor <- max(df_asv$Conteo) / max(df_otu$Conteo)

ggplot() +
  geom_line(data = df_asv, aes(x = Nivel, y = Conteo, group = 1),
            color = "#1b9e77", linewidth = 1.5, alpha = 0.7) +
  geom_point(data = df_asv, aes(x = Nivel, y = Conteo),
             color = "#1b9e77", size = 3) +
  geom_line(data = df_otu, aes(x = Nivel, y = Conteo * scale_factor, group = 1),
            color = "#d95f02", linewidth = 1.5, alpha = 0.7) +
  geom_point(data = df_otu, aes(x = Nivel, y = Conteo * scale_factor),
             color = "#d95f02", size = 3) +
  scale_y_continuous(
    name = "ASVs",
    sec.axis = sec_axis(~ . / scale_factor, name = "OTUs")
  ) +
  labs(
  ...
  theme(
    ...
    axis.title.y.left = element_text(face = "bold", color = "#1b9e77"),
    axis.text.y.left = element_text(color = "#1b9e77"),
    axis.title.y.right = element_text(face = "bold", color = "#d95f02"),
