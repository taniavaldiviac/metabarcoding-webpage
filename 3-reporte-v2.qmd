...
::: {.callout-note icon="false"}
**Curación geográfica en el MOCK**

Aplicamos la misma curación geográfica al MOCK que a las muestras ambientales. Esto significa que especies del control positivo que no estén documentadas en el Golfo de California pueden aparecer como `Género sp.` o perder la asignación a especie. Tenlo en cuenta al interpretar los resultados: una especie "no detectada" podría haber sido identificada por DADA2 pero eliminada por el filtro geográfico.
:::
...
# Para el MOCK usamos la misma taxonomía curada geográficamente (ps)
ASV_df_mock <- as.data.frame(phyloseq::otu_table(ps)) %>%
  tibble::rownames_to_column("seqID") %>%
  tidyr::pivot_longer(cols = -seqID, names_to = "Sample_name", values_to = "nReads")

TAXA_df_mock <- as.data.frame(phyloseq::tax_table(ps)) %>%
  tibble::rownames_to_column("seqID")

species_site_mock <- ASV_df_mock %>%
  dplyr::left_join(TAXA_df_mock, by = "seqID") %>%
  dplyr::filter(!is.na(Species)) %>%
  dplyr::group_by(Sample_name, Species) %>%
  dplyr::summarise(nReads = sum(nReads), .groups = "drop") %>%
  tidyr::pivot_wider(names_from = Sample_name, values_from = nReads, values_fill = 0)
