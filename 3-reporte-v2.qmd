...
Este reporte presenta el flujo completo de análisis ecológico de comunidades de peces en el Golfo de California a partir de datos de metabarcoding del gen 12S rDNA. Partiendo de los archivos `.RData` generados por el pipeline DADA2, construimos el objeto *phyloseq*, evaluamos la calidad de los datos y aplicamos análisis de diversidad alfa, composición taxonómica y diversidad beta. En lugar de rarefacción, usamos **abundancias relativas** para los análisis composicionales e **índices robustos** (Chao1, ACE) para estimar riqueza, siguiendo las mejores prácticas actuales para estudios de eDNA en ecología de comunidades.
...
```{r rutas}
# Buscar el directorio metabarcoding-code en el servidor
mc_dir <- system("find ~ -maxdepth 3 -type d -name 'metabarcoding-code' 2>/dev/null | head -1", intern = TRUE)
if (length(mc_dir) == 0 || mc_dir == "") stop("No se encontró el directorio metabarcoding-code")
setwd(mc_dir)
message("Working directory: ", mc_dir)
...
::: {.callout-warning}
**Bug conocido:** la columna `sample_id` en `metadata.csv` puede estar vacía (NA). Esto causa que `sample_meta` se cargue sin rownames válidos y todo el análisis falle con NAs. Si te pasa, restaura el archivo original desde GitHub:
...
El pipeline usa la columna `rename_to` para alinear los nombres de muestra, así que el análisis funciona aunque `sample_id` esté vacío.
:::
...
## 3. Construcción del objeto phyloseq
...
```{r vsearch-run}
# Ejecutar vsearch desde R
vsearch_similarity <- 0.97
fasta_in  <- "final_data/otus_vsearch/fish_ASV.fasta"
uc_out    <- paste0("final_data/otus_vsearch/otus_", vsearch_similarity, ".uc")
rep_out   <- paste0("final_data/otus_vsearch/otus_", vsearch_similarity, "_oturep.fasta")
log_out   <- paste0("final_data/otus_vsearch/otus_", vsearch_similarity, ".log")

vsearch_cmd <- paste(
  "vsearch --cluster_size", fasta_in,
...
message("Ejecutando: ", vsearch_cmd)
system(vsearch_cmd)
message("VSEARCH completado.")
```
...
uc.df <- readr::read_tsv(uc_out, col_names = uc_names, show_col_types = FALSE) |>
...
# Tabla de conteos original en formato largo
conteos.ini <- data.frame(phyloseq::otu_table(ps.asv), stringsAsFactors = FALSE)
...
MD_otu   <- phyloseq::sample_data(oturep.md)

ps.otu <- phyloseq::phyloseq(ASV_otu, TAXA_otu, SEQ_otu, MD_otu)
ps.otu
```

### Comparar resolución taxonómica: ASVs vs OTUs

```{r vsearch-comparar, fig.width=9, fig.height=5}
...
ggplot(df_comp, aes(x = Nivel, y = Conteo, group = Estrategia, color = Estrategia)) +
  geom_line(linewidth = 1.5, alpha = 0.7) +
  geom_point(size = 3) +
  scale_color_manual(values = c("ASVs" = "#1b9e77", "OTUs" = "#d95f02")) +
  labs(
    title = "Número de taxones identificados: ASVs vs OTUs (97%)",
...
::: {.callout-note icon="false"}
**Interpretación:** La diferencia entre ASVs y OTUs refleja cuánta variación intraespecífica (haplotipos, alelos) captura el análisis por ASVs. A niveles taxonómicos altos (Phylum, Class) la diferencia es mínima; a nivel de Genus/Species puede ser notable. En cada nivel puede haber asignaciones `NA` que cuentan como 1 taxón.
:::
...
### Distribución de longitudes de ASVs

Las ASVs deben tener una longitud consistente con el amplicón esperado (~62 bp para 12S). Secuencias muy cortas o muy largas pueden ser artefactos de ensamblaje.
...
::: {.callout-note icon="false"}
**¿Por qué no aplicar la curación geográfica al MOCK?**

El MOCK es un control de performance del pipeline, no una muestra ambiental del Golfo de California. Contiene especies de laboratorio — algunas pueden no estar en la lista de peces del GC. Si aplicáramos la curación geográfica, esas especies quedarían como `NA` y parecería que no fueron detectadas, cuando en realidad sí lo fueron. Para evaluar el pipeline correctamente, usamos la taxonomía directa del clasificador DADA2 (pre-curación).
:::

```{r mock-deteccion}
# Tabla de abundancias curada (para CSV export y análisis principales)
...
# Para el MOCK usamos taxonomía PRE-curación (taxa_df, antes de filtro geográfico)
# para recuperar todas las especies del control positivo, incluso si no están en la lista del GC
ASV_df_mock <- as.data.frame(ASV_mat) %>%
  tibble::rownames_to_column("Sequence") %>%
  tidyr::pivot_longer(cols = -Sequence, names_to = "Sample_name", values_to = "nReads")

TAXA_df_precur <- taxa_df %>%
  tibble::rownames_to_column("Sequence")

species_site_mock <- ASV_df_mock %>%
  dplyr::left_join(TAXA_df_precur, by = "Sequence") %>%
  dplyr::filter(!is.na(Species)) %>%
  dplyr::group_by(Sample_name, Species) %>%
  dplyr::summarise(nReads = sum(nReads), .groups = "drop") %>%
  tidyr::pivot_wider(names_from = Sample_name, values_from = nReads, values_fill = 0)
