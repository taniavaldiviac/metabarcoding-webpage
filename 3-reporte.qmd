...
# Cargar el archivo phyloseq_inputs generado por el pipeline DADA2
rdata_file <- list.files(ruta_rdata, full.names = TRUE, pattern = "phyloseq_inputs\\.RData$")
if (length(rdata_file) == 0) stop("No se encontró *phyloseq_inputs.RData en ", ruta_rdata)
if (length(rdata_file) > 1) {
  message("Múltiples archivos encontrados, usando el más reciente:")
...
### Guardar objeto phyloseq antes de colapsar (para VSEARCH)

Antes de agrupar las ASVs por especie, guardamos el objeto phyloseq con las ASVs individuales. Este objeto es el que se usará en el Día 4 para el agrupamiento en OTUs con VSEARCH.

```{r Guardar phyloseq para VSEARCH}
...
message("Phyloseq (ASVs sin colapsar) guardado en final_data/otus_vsearch/")
```
